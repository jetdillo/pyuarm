...
#if DEVICE == 'swift':
#    from . import swift_protocol as protocol
#else:
...
class UArm(object):
    ...
    def __init__(self, port_name=None, logger=None, debug=False, block=True, timeout=0.1):
        ...
        if self.proto_version == "none": 
           ...
           printf("Detected uArm model {}".format(self.proto_version),INFO)
           self.block = block
           self.timeout = timeout
           self.port = get_port_property(port_name)
           #set __isReadyEarly to True since finding the model ID means we can communicate with it
           #and we've already eaten the protocol.READY that __connect will be looking for
           self.__isReadyEarly = True
           self.connect()
    ...
    def connect(self):
        ...
        self.__isConnected = False
        #Allow isReady if arm was ID'ed
        if self.__isReadyEarly:
           self.__isReady = True
        else:
...
# -------------------------------------------------------- Get Commands -----------------------------------------------#
    ...
    def get_rom_data(self, address, data_type):
...
# -------------------------------------------------------- Set Commands -----------------------------------------------#
    ...
    def set_rom_data(self, address, data, data_type):
...
# ---------------------------------------------------- Report Commands -----------------------------------------------#
    ...
    def set_proto(self,proto):
       ...
       if proto == "metal":
          import protocol
       if proto == "swift":
          import swift_protocol as protocol
